...
fn op_parser(input : &str) -> IResult<&str, Op> {
    alt((value(Nop,tag("nop")),value(Acc,tag("acc")),value(Jmp,tag("jmp"))))(input)
}
...
fn num_parser(input : &str) -> IResult<&str, i32> {
    ...
    map_res(
	digit1,
	move |x : &str | -> Result<i32,ParseIntError> {
	    let y : i32 = x.parse::<i32>()?;
	    if sign {Ok(y)} else {Ok(-y)}
	}
    )(input)
}
...
fn main() {
    ...
    let instructions : Vec<_> = lines().map(|x| parser(x).expect("parsing failed").1).collect();
