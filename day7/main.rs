...
fn numbered_bag_parser(input: &str) -> IResult<&str, (Bag, u32)> {
    map(
	separated_pair(number,tag(" "),bag_parser),
	|(n, b)| (b , n)
    )(input)
}
...
fn second_parser(input: &str) -> IResult<&str, HashMap<Bag,u32>> {
	...
	alt(
	    ...
	    map(
		separated_list1(tag(", "), numbered_bag_parser),
		|x| x.iter().cloned().collect::<HashMap<_,_>>()
	    ))
	)
	,tag(".")
    )(input)
...
fn can_have_gold(dict : &HashMap<Bag,HashMap<Bag,u32>>, b : &Bag) -> bool {
	...
	x.keys().any(|y| y == "shiny gold" || can_have_gold(dict,y))
    }
    else {false}
}
...
fn bags_contained(dict : &HashMap<Bag,HashMap<Bag,u32>>, b : &Bag) -> u32 {
    ...
    bags_inside.map(|(bag, number)| (1 + bags_contained(dict,bag)) * number).sum()
}
...
fn main() {
    ...
    let dict : HashMap<Bag,HashMap<Bag,u32>> = lines().map(|x| line_parser(x).expect("Bit of a yikes").1).collect();

    println!("{}", dict.len());

    println!("Part 1: {}",dict.keys().filter(|x| can_have_gold(&dict,x)).count());

    println!("Part 2: {}",bags_contained(&dict,&"shiny gold".to_string()))
}
